...
function detectRuntime(): AppRuntime {
  ...
  // Determine actual mobile device vs resized desktop
  // A device is likely a true mobile device if it has a mobile UA or reports coarse pointer / touch (as primary on small screen)
...
export function useAppRuntime(): AppRuntime {
  ...
  useEffect(() => {
    ...
    // Asynchronously check SW registration
    if ("serviceWorker" in navigator) {
      // Check immediately
      navigator.serviceWorker.getRegistration().then((reg) => {
        if (reg) setRuntime((prev) => ({ ...prev, serviceWorkerRegistered: true }));
      });
      // Also wait for it to be ready (which happens after Serwist registers it)
