    ...
    <div className="p-4 sm:p-5 rounded-2xl bg-[var(--color-surface)] border-2 border-[var(--color-border)] shadow-[3px_3px_0px_0px_var(--color-border)] space-y-4">
      ...
      <div className="flex items-center justify-between pb-3 border-b-2 border-dashed border-[var(--color-border)]">
        <div>
          <div className="flex items-center gap-2">
            <span className="p-1 rounded-lg bg-indigo-400 text-black border border-black/20">
        ...
        <div className="text-[9px] font-black uppercase px-2 py-0.5 rounded-full bg-[var(--color-bg)] border border-[var(--color-border)] text-gray-400">
          ...
          return (
            ...
            >
              ...
              <div className="text-center min-h-[14px]">
                {day.amount > 0 && (
                  <span className="text-[9px] font-black font-numbers text-gray-400 group-hover:text-[var(--color-primary)] truncate block max-w-[42px]">
              ...
              {/* Bar Container */}
              <div className="w-full h-24 bg-[var(--color-bg)] border-2 border-[var(--color-border)] rounded-xl relative overflow-hidden flex items-end p-1">
                <motion.div
                  initial={{ height: 0 }}
                  ...
                  className={`w-full rounded-lg transition-colors ${
                    isPeak
                      ? "bg-[var(--color-primary)] border border-black/30"
                      : isWeekend
              ...
              <div className="text-center">
                <span
                  className={`text-[10px] font-black uppercase tracking-wider block ${
                    isPeak
                      ? "text-[var(--color-primary)] font-black underline"
                      : isWeekend
                      ? "text-indigo-400"
                      : "text-gray-400"
                  }`}
                ...
                <span className="text-[8px] font-bold text-gray-500 block">
                  {day.txCount} tx
                </span>
              </div>
            </motion.div>
      ...
      <div className="pt-2 border-t border-[var(--color-border)] space-y-1.5">
        <div className="flex justify-between text-[10px] font-black uppercase tracking-wider text-gray-400">
          <span className="text-emerald-400">Weekdays: {formatCurrency(weekdayTotal)} ({weekdayPct.toFixed(0)}%)</span>
          <span className="text-indigo-400">Weekends: {formatCurrency(weekendTotal)} ({weekendPct.toFixed(0)}%)</span>
        </div>
