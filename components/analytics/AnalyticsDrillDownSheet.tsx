    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[120] flex items-end justify-center pointer-events-auto">
        ...
        >
          ...
          {/* Header */}
          <div className="p-4 sm:p-5 border-b-2 border-[var(--color-border)] bg-[var(--color-surface)] flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0">
              <div
                className="w-10 h-10 rounded-xl flex items-center justify-center text-black border border-black/20 shrink-0"
                style={{ backgroundColor: filter.color || "var(--color-primary)" }}
              >
                <HeaderIcon className="w-5 h-5 stroke-[2.5px]" />
              </div>
              <div className="min-w-0">
                <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 block">
                  Filtered Transactions ({transactions.length})
                </span>
                <h3 className="text-base font-display font-black text-[var(--color-text)] truncate">
                  {filter.name}
                </h3>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                type="button"
                onClick={onClose}
                className="p-2 rounded-xl bg-[var(--color-bg)] border-2 border-[var(--color-border)] text-gray-400 hover:text-[var(--color-text)] hover:border-gray-500 cursor-pointer"
              >
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2.5 max-h-[60vh]">
              ...
              transactions.map((txn) => {
                ...
                return (
                  <motion.div
                    ...
                    className="p-3 rounded-2xl bg-[var(--color-surface)] border-2 border-[var(--color-border)] hover:border-[var(--color-primary)] transition-all cursor-pointer flex items-center justify-between gap-3 group"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5 text-[10px] font-bold text-gray-400">
                          ...
                          {txn.splits && txn.splits.length > 0 && (
                            <span className="px-1 py-0.2 bg-blue-500/20 text-blue-400 border border-blue-500/40 rounded text-[9px]">
