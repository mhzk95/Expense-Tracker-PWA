  ...
  return (
    <div className="space-y-4 pt-6">
      <div className="flex items-center justify-between px-1">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2 border-2 border-transparent hover:border-[var(--color-border)] rounded-lg text-[var(--color-text)] hover:bg-[var(--color-surfaceHover)] transition-colors"
            title="Refresh Logs"
          >
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-red-50 border-2 border-[var(--color-border)] rounded-[16px]  p-4 flex flex-col justify-between h-24 relative overflow-hidden">
        ...
        <div className="bg-amber-50 border-2 border-[var(--color-border)] rounded-[16px]  p-4 flex flex-col justify-between h-24 relative overflow-hidden">
          <div className="absolute right-2 top-2 opacity-10">
        ...
        <div className="bg-emerald-50 border-2 border-[var(--color-border)] rounded-[16px]  p-4 flex flex-col justify-between h-24 relative overflow-hidden">
          <div className="absolute right-2 top-2 opacity-10">
      ...
      {/* Sync Actions */}
      {syncQueueCount > 0 && (
        <div className="bg-amber-100 border-2 border-[var(--color-border)]  rounded-[16px] p-4 flex items-center justify-between">
      ...
      <div className="bg-[var(--color-surface)] border-2 border-[var(--color-border)]  rounded-[16px] overflow-hidden">
        <div className="px-4 py-3 bg-[var(--color-surfaceHover)] border-b-[3px] border-[var(--color-border)] flex items-center justify-between">
              ...
              return (
                <div key={log.id} className="text-xs">
                  ...
                  >
                    <div className="mt-1">
                      {log.level === "error" ? (
                        <div className="h-3 w-3 rounded-full bg-red-500 border-2 border-[var(--color-border)] animate-pulse " />
                      ) : log.level === "warning" ? (
                        <div className="h-3 w-3 rounded-full bg-amber-400 border-2 border-[var(--color-border)] " />
                      ) : (
                        <div className="h-3 w-3 rounded-full bg-blue-500 border-2 border-[var(--color-border)] " />
                      )}
                    </div>
                  ...
                  {isExpanded && (
                    <div className="px-9 pb-4 pt-3 bg-[var(--color-surfaceHover)] text-[10px] font-mono font-bold text-gray-700 space-y-2 overflow-x-auto border-t-2 border-[var(--color-border)]">
            ...
            {logs.length > 5 && (
              <div className="p-3 bg-[var(--color-bg)] flex justify-center border-t-2 border-[var(--color-border)]">
                <button
                  ...
                  className="w-full py-2 text-xs font-black uppercase tracking-widest text-[var(--color-text)] hover:bg-[var(--color-surfaceHover)] rounded-[8px] flex items-center justify-center gap-1.5 transition-colors border-2 border-transparent hover:border-[var(--color-border)]"
                >
                  {showAllLogs ? (
                    <>
